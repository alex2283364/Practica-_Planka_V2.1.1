...
check_docker()    { command -v docker >/dev/null 2>&1 || die "Docker not installed"; }
check_container() { docker compose ps | grep -q "$CONTAINER_NAME" || die "PostgreSQL container not running"; }
...
SQL
  ...
  # ─── 4. Менеджеры проектов: все × все ─────────────────────────────
  exec_sql <<'SQL'
insert
into
public.project_manager (id,
project_id,
user_id,
created_at,
updated_at)
...
SQL
  ...
  # ─── 6. Участники досок (все — editors) ────────────────────────────
  exec_sql <<'SQL'
insert
into
public.board_membership (id,
project_id,
board_id,
user_id,
"role",
...
SQL
  ...
  # ─── 7. Списки (БЕЗ color! Только id, board_id, type, position, name) ─
  exec_sql <<'SQL'
insert
into
public.list (id,
...
SQL
  ...
  # ─── 9. Карточки (БЕЗ list_changed_at!) ───────────────────────────
  exec_sql <<'SQL'
insert
into
public.card (id,
...
main() {
  log_info "Planka Test Data Loader (v3 - fixed schema compatibility)"
  check_docker
  check_container
  load_env
  load_test_data
