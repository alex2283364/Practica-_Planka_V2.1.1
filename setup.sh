...
# -----------------------------------------------------------------------------
echo "[6/7] Создание базы данных и выполнение миграций..."

if [ -f "./load-data.sh" ]; then
    chmod +x ./load-data.sh
    if ./load-data.sh load; then
        ...
        echo "ПРЕДУПРЕЖДЕНИЕ: load-data.sh завершился с ошибкой."
        echo "Попробуйте загрузить данные вручную через веб-интерфейс."
    fi
else
    echo "ПРЕДУПРЕЖДЕНИЕ: load-data.sh не найден. Пропускаем загрузку тестовых данных."
fi
